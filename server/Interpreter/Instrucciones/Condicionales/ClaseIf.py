...
class ClaseIf(Instruccion, Expresion):
    ...
    def getTipo(self, driver, ts, errores):
        ...
        if self.cuerpo.getTipo(driver, ts_local, errores) != self.Else.getTipo(driver, ts_local, errores):
            ...
            raise Exception({"tipo":"Semantico", "descripcion":f"los valores a retornar en if deben ser del mismo tipo", "linea": str(self.linea), "columna":str(self.columna), "ambito": ts_local.env})
